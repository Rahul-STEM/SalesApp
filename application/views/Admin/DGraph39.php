    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Status Wise Task Plan/Inisated/Updated</h5></center><hr>
...
<div style="width: 80%; margin: auto;">
   <canvas id="combinedChartID1"></canvas>
    ...
    $ed=date('Y-m-d');?>
        ...
        var combinedData1 = {
            labels: [<?php $status = $this->Menu_model->get_status(); $i=1; foreach($status as $st){ $stid = $st->id;?> '<?=$st->name?>', <?php } ?>],
            datasets: [
                {
                    label: 'Task Planned',
                    ...
                    data: [
                        <?php 
                        $status = $this->Menu_model->get_status(); $i=1; foreach($status as $st){ $stid = $st->id;
                        $task = $this->Menu_model->get_tasktypeupdatebysp($uid,$sd,$ed,$stid);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                
                {
                    ...
                    backgroundColor: ['blue','green','yellow','purple','orange','pink','brown','cyan','magenta','teal','lime','violet','indigo','red'],
                    data: [
                        <?php 
                        $status = $this->Menu_model->get_status(); $i=1; foreach($status as $st){ $stid = $st->id;
                        $task = $this->Menu_model->get_tasktypeupdatebysi($uid,$sd,$ed,$stid);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                {
                    label: 'Task Updated',
                    backgroundColor: ['green','yellow','purple','orange','pink','brown','cyan','magenta','teal','lime','violet','indigo','red','blue'],
                    data: [
                        <?php 
                        $status = $this->Menu_model->get_status(); $i=1; foreach($status as $st){ $stid = $st->id;
                        $task = $this->Menu_model->get_tasktypeupdatebysu($uid,$sd,$ed,$stid);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
