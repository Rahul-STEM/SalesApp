                ...
                <div class="container-fluid">
                    <div class="card">
                        <div class="card-header">
                            <center>
                                <h5>Time Wise Task Plan/Initiated/Updated</h5>
                            </center>
                        </div>
                        <div class="card-body FilterSection">
                            <form method="POST" action="<?= base_url(); ?>GraphNew/TimeSlotWiseTaskAnalysis/<?php ?>">
                                <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card card-body">
                                <canvas id="TimeSlotWiseTask" style="width: 100%; height: 300px;"></canvas>
...
<script>
    var combinedData1 = {
        ...
        labels: [
            <?php 
                ...
                foreach($times as $ts){
                    ...
                    $t1 = $ts->time1;$t2 = $ts->time2;?> '<?=$ts->time1?> to <?=$ts->time2?>', 
        ...
        datasets: [
            {
                ...
                data: [
                    <?php 
                    $times = get_timeslot(); $i=1; foreach($times as $ts){ $t1 = $ts->time1;$t2 = $ts->time2;
                    // $task = $this->Menu_model->get_tasktypeupdatebytp($uid,$sd,$ed,$t1,$t2);
                    $plantask = get_taskplannedSlotwise($selected_users,$Selected_userType,$sdate,$edate,$t1,$t2,$selected_partnerType,$selected_category);

                    foreach($plantask as $pts){
                    ?>
                    <?=$pts->count?>,
                    <?php }} ?>],
                    stack: 'Stack 0'
            ...
            {
                label: 'Task Initiated',
                backgroundColor: ['blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue'],
                data: [
                    <?php 
                    $times = $this->Menu_model->get_timeslot(); $i=1; foreach($times as $ts){ $t1 = $ts->time1;$t2 = $ts->time2;
                    // $task = $this->Menu_model->get_tasktypeupdatebyti($uid,$sd,$ed,$t1,$t2);
                    $initask = get_taskinitiatedSlotwise($selected_users,$Selected_userType,$sdate,$edate,$t1,$t2,$selected_partnerType,$selected_category);

                    foreach($initask as $its){
                    ?>
                    <?=$its->count?>,
                    <?php }} ?>],
                    stack: 'Stack 2'
            ...
            
            {
                ...
                data: [
                    <?php 
                    $times = $this->Menu_model->get_timeslot(); $i=1; foreach($times as $ts){ $t1 = $ts->time1;$t2 = $ts->time2;
                    // $task = $this->Menu_model->get_tasktypeupdatebytu($uid,$sd,$ed,$t1,$t2);
                    $Updatedtask = get_taskupdatedSlotwise($selected_users,$Selected_userType,$sdate,$edate,$t1,$t2,$selected_partnerType,$selected_category);

                    foreach($Updatedtask as $uts){
                    ?>
                    <?=$uts->count?>,
                    <?php }} ?>],
                    stack: 'Stack 3'
        ...
        var combinedCtx = document.getElementById("TimeSlotWiseTask").getContext('2d');
        var combinedChart = new Chart(combinedCtx, {
            ...
            options: {
                plugins: {
                    title: {
                        display: true,
                        text: 'Planned Vs Initiated vs Updated Task'
                    },
                },
                scales: {
                    x: {
