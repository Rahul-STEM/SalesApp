                            ...
                            <div class="table-responsive">     
                                <table id="example1" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <tbody>
                                        <?php 
                                        $i=1;
                                        $bd = $this->Menu_model->get_pst_team($uid);
                                        foreach ($bd as $b){
                                        $bdid = $b->user_id;
                                        $mdata=$this->Menu_model->get_bdrequest_PSTTEAM($bdid,$code);
                                        foreach($mdata as $dt){
                                      ...
                                      <tr>
                                         ...
                                         <td><?php if($attech[0]->att==''){echo 'Not Available';}
                                            ...
                                            if($atte[$k]!=''){
                                            ?>
                                            <img src="https://stemoppapp.in/<?=$atte[$k]?>" height="200px" alt="" class="img-fluide src">
                                         ...
                                         $timed = $this->Menu_model->timediff($newddate, $olddate);?>
                                             ...
                                             <b><?=$timed?></b><br><b>Detail:</b> <?=$log->detail?><?=$log->remark?><hr>
...
<div id="bdropen" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-standard-title" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-standard-title1"></h5>
                ...
                <div class="modal-body">
                    <div class="card card-form col-md-12">
                       <div class="card-header bg-info">Request Re-Open</div>
                        <div class="col-lg-12 card-body">
                           <?=form_open('Menu/bdropen')?>
                           <input type="hidden" id="rrrid" name="rrrid">
                           <textarea rows="10" name="rccomment" class="form-control"></textarea><hr>
                           <button type="submit" class="btn btn-primary mt-3" onclick="this.form.submit(); this.disabled = true;">Submit</button>
                           </form>
                        </div>
                        </div>
...
$('[id^="add_open"]').on('click', function() {
        var rrid = this.value;
    $('#bdropen').modal('show');
    document.getElementById("rrrid").value = rrid;
});
