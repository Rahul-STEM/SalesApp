  ...
  <div class="content-wrapper">
    ...
    <div class="card">
              <div class="card-header">
                <h3 class="card-title">Handover to Operations For CID - <?= $cid ?></h3>
              </div>
              <!-- /.card-header -->
              <!-- form start --> 
              <form action="<?=base_url();?>Menu/bdHandover" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        <div class="col-lg-4 p-3">
                              <div class="form-group">
                                ...
                                <input type="hidden" class="form-control" name="proid" value="<?=$proid?>" required readonly>
                                <input type="hidden" class="form-control" name="bdid" value="<?=$uid?>" required readonly>
                                <input type="hidden" class="form-control" name="cid" value="<?= $cid ?>" required readonly>
                        ...
                        <div class="col-lg-4 p-3">
                            <div class="form-group">
                              ...
                              <div class="form-group">
                                ...
                                <select class="form-control" name="infrastructure" id="infrastructure" placeholder="School Identification by" required>
                                    <option value="" selected disabled>Select School Infrastructure for MSC</option>
                        ...
                        <div class="col-lg-4 p-3">
                              ...
                              <div class="form-group">
                                ...
                                <div class="invalid-feedback">Please provide a Information.</div>
                                <div class="valid-feedback">Looks good!</div>
                                <div class="uploaded-image" id="imgblah">
                ...
                <?php  if(isset($gtscl)){?>
                  <h4 class="text-left"><u> Please Select School :</u></h4>
                  <?php foreach($gtscl as $gtscldtld){ ?>
                  <div class="row schollrow">
                  <table class="table table-striped">
                        ...
                        <tr>
                          <td>
                    <input class="noofschoolcnt" value="<?= $gtscldtld->id; ?>" name="slctschoolid[]" type="checkbox" >
                          </td>
                            <td><?= $gtscldtld->sname; ?></td>
                            <td><?= $gtscldtld->saddress; ?></td>
                            <td><?= $gtscldtld->scity; ?></td>
                            <td><?= $gtscldtld->sstate; ?></td>
                            <td><?= $gtscldtld->contact_name; ?></td>
                            <td><?= $gtscldtld->contact_no; ?></td>
                            <td><?= $gtscldtld->designation; ?></td>
                ...
                <div class="p-3" id="sdetailclient">
                 
                <?php  
                // var_dump($gtscl);
                if(!is_null($gtscl)){
                  $schoolcount = count($gtscl);
                }
                  if(empty($schoolcount) && is_null($gtscl)){
                    $schoolcount = 5;
                  }
                ...
                for($i=1;$i<=$schoolcount;$i++) {
                  ...
                  // die;
                   ?>
                    <div class="row" id="clientaddschool">
                        ...
                        <input type="text" name="scontact[]" class="form-control col-3"  placeholder="Contact Name">
...
document.getElementById("sdetailclient").style.display = "none";
...
if(ideby=='Client'){
  ...
  document.getElementById("sdetailclient").style.display = "block";
  document.getElementById("sdetail").style.display = "none";
} else {
...
document.getElementById("imgblah").style.display = "none";
    filname.onchange = evt => {
  const [file] = filname.files
  if (file) {
    document.getElementById("imgblah").style.display = "block";
    blah.src = URL.createObjectURL(file)
  }
}

  $('.noofschoolcnt').change(function() {
        var totalCheckboxes = $('.noofschoolcnt').length;
        var checkedCount = $('.noofschoolcnt:checked').length;
        console.log("Checked noofschoolcnt: " + checkedCount);
        var noofschool = $('#noofschool').val();
        if(noofschool == 0){
          alert("Please Add Number of Schools First");
          $(this).prop('checked', false);
        }else{
          if(noofschool ==  checkedCount){
          var leftoverCheckboxes = totalCheckboxes - checkedCount;
          $('.noofschoolcnt').not(':checked').prop('disabled', leftoverCheckboxes >= 0 && checkedCount < totalCheckboxes);
          }else{
            var leftoverCheckboxes = totalCheckboxes - checkedCount;
          $('.noofschoolcnt').not(':checked').prop('disabled',false, leftoverCheckboxes >= 0 && checkedCount < totalCheckboxes);
          }
        }
    });
...
// var slctval = $('#spd_identify_by').val();
